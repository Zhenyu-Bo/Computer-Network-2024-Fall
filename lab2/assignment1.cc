...
static Ptr<OutputStreamWrapper> cWndStream;  // stream of cwnd
static Ptr<OutputStreamWrapper> ssThreshStream; // stream of slow start threshold
static Ptr<OutputStreamWrapper> rttStream;  // stream of RTT
static Ptr<OutputStreamWrapper> rtoStream;  // stream of RTO(Re-Transmission TimeOut)
static Ptr<OutputStreamWrapper> nextTxStream;  // stream of next transmission sequence number
static Ptr<OutputStreamWrapper> nextRxStream;  // stream of next reception sequence number
static Ptr<OutputStreamWrapper> inFlightStream;  // stream of bytes in flight (the bytes that have been sent but not yet acknowledged)
...
int main (int argc, char *argv[])
{
  ...
  delete temp_header;
  uint32_t tcp_adu_size = mtu_bytes - 20 - (ip_header + tcp_header); // ADU = MTU - 20 - IP Header - TCP Header
  ...
  // LocalLink.SetDeviceAttribute("ReceiveErrorModel", PointerValue(&error_model));
  ...
  uint32_t size = static_cast<uint32_t>((std::min (access_b, bottle_b).GetBitRate () / 8.0) *
    ((access_d + bottle_d) * 2).GetSeconds ());
  ...
  for (uint16_t i = 0; i < sources.GetN (); i++)
    {
      AddressValue remoteAddress (InetSocketAddress (sink_interfaces.GetAddress (i, 0), port)); // 接收节点地址
      Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (tcp_adu_size)); // 设置TCP套接字段大小
      BulkSendHelper ftp ("ns3::TcpSocketFactory", Address ()); // 创建BulkSendHelper，用于配置和安装BulkSendApplication
